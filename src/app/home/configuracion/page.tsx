...
function SectionLabel({ text }: { text: string }) {
    return (
        <p className="text-sm font-semibold text-blue-grey uppercase tracking-wide px-4 mb-2">
            {text}
        </p>
    );
}
...
function SettingRow({
    ...
    return (
        <div
            ...
            onClick={onClick}
        >
            <div className="w-12 h-12 rounded-full bg-main-green-bg flex items-center justify-center shrink-0">
                {icon}
            </div>
            <div className="flex-1 min-w-0">
                <p className="text-base font-medium text-dark-blue-grey">{label}</p>
                {sublabel && <p className="text-sm text-blue-grey mt-0.5">{sublabel}</p>}
            </div>
...
function IconUser() {
    return (
        <svg className="w-6 h-6 text-main-green" fill="currentColor" viewBox="0 0 24 24">
...
function IconChevron() {
    return (
        <svg className="w-5 h-5 text-blue-grey" fill="currentColor" viewBox="0 0 24 24">
        ...
        <div className="min-h-screen bg-surface">
                ...
                <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center px-4 backdrop-blur-sm">
                    <div className="bg-card rounded-2xl w-full max-w-sm overflow-hidden shadow-xl animate-scaleIn">
                        <div className="p-5 border-b border-surface-variant flex justify-between items-center bg-main-green/5">
                            <h3 className="text-lg font-bold text-dark-blue-grey">
                                {lang === "ca" ? "Afegir MO" : "Añadir MO"}
                            </h3>
                            <button
                                onClick={() => setShowModalMO(false)}
                                className="w-9 h-9 flex items-center justify-center rounded-full bg-surface-variant text-blue-grey hover:bg-main-green hover:text-white transition-colors"
                            >
                                <svg
                                    className="w-5 h-5"
                                    fill="none"
                        ...
                        <div className="p-5">
                            <p className="text-base text-blue-grey mb-4">
                                {lang === "ca"
                                    ? "Introdueix el codi de la nova explotació (ex: ES123...):"
                                    : "Introduce el código de la nueva explotación (ej: ES123...):"}
                            </p>
                            <input
                                type="text"
                                value={inputMO}
                                onChange={(e) => setInputMO(e.target.value.toUpperCase())}
                                className="w-full border-2 border-surface-variant rounded-xl px-4 py-3 text-base uppercase focus:border-main-green outline-none font-mono bg-surface text-dark-blue-grey"
                                placeholder="00XXAM"
                                autoFocus
                            />
                            <button
                                ...
                                className="w-full bg-main-green text-white rounded-xl py-3.5 text-base font-semibold mt-5 disabled:opacity-40 transition-opacity"
                            >
            ...
            {exito && (
                <SuccessModal
                    ...
                    mensaje={
                        lang === "ca"
                            ? "L'explotació ha estat validada i guardada correctament."
                            : "La explotación ha sido validada y guardada correctamente."
                    }
                    boton="Acceptar"
