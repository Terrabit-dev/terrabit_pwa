...
export default function BorradoresPage() {
    ...
    const handleEliminar = async (id: number) => {
        const confirmar = confirm(
            lang === "ca"
                ? "Segur que vols esborrar aquest esborrany?"
    ...
    return (
        <div className="min-h-screen bg-surface pb-24">
            ...
            <div className="px-4 pt-4 pb-2">
                <div className="flex items-center gap-2 bg-card border border-surface-variant rounded-xl px-4 py-3 focus-within:border-main-green transition-colors shadow-sm">
                    <svg
            ...
            <div className="px-4 py-2 flex items-center justify-between">
                {isEditing ? (
                    <button
                        ...
                        className="text-sm font-semibold text-main-green flex items-center gap-2"
                    >
                        <div
                            className={`w-5 h-5 rounded border flex items-center justify-center transition-colors ${
                                selectedIds.size === listaFiltrada.length && listaFiltrada.length > 0
                ...
                <div className="flex gap-4">
                    {!isEditing && (
                        <button
                            ...
                            disabled={cargando}
                            className="text-sm text-blue-grey font-medium disabled:opacity-40 hover:text-main-green"
                        >
                    ...
                    {listaFiltrada.length > 0 && (
                        <button
                            ...
                            className={`text-sm font-bold transition-colors ${
                                isEditing ? "text-error-red" : "text-main-green"
                            }`}
                        >
                            {isEditing
                                ? lang === "ca"
                    ...
                    {listaFiltrada.map((registro) => {
                        ...
                        return (
                            <div
                                ...
                                className={`bg-card rounded-2xl p-5 shadow-sm border transition-all duration-200 flex items-center gap-4 ${
                                    isEditing ? "cursor-pointer" : ""
                                } ${
                                    isSelected ? "border-main-green bg-main-green/5" : "border-surface-variant"
                                }`}
                            >
                                {/* Checkbox animado */}
                                {isEditing && (
                                    ...
                                    {!isEditing && (
                                        <div className="flex gap-2 pt-2 border-t border-surface-variant">
                                            <button
                                                ...
                                                className="flex-1 border border-error-red/20 text-error-red rounded-xl py-3 text-sm font-semibold hover:bg-error-red/5 transition-colors"
                                            >
                                            ...
                                            <button
                                                onClick={(e) => {
                                                    e.stopPropagation();
                                                    handleContinuar(registro);
                                                }}
                                                className="flex-1 bg-main-green text-white rounded-xl py-3 text-sm font-semibold hover:opacity-90 transition-opacity"
                                            >
            ...
            {isEditing && selectedIds.size > 0 && (
                <div className="fixed bottom-0 left-0 right-0 p-4 bg-white border-t border-surface-variant shadow-[0_-10px_20px_rgba(0,0,0,0.05)] z-50 animate-slideUp">
                    <button
                        ...
                        className="w-full bg-error-red text-white font-bold py-3.5 rounded-xl shadow-sm hover:opacity-90 transition-opacity flex items-center justify-center gap-2 text-base"
                    >
                        <svg
                            className="w-5 h-5"
                            fill="none"
                        ...
                        {lang === "ca"
                            ? `Esborrar ${selectedIds.size} seleccionats`
